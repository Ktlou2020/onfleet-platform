...
const SCORECARD_CRITICAL_ALERT_TYPES = ['panic', 'tamper', 'power_disconnect', 'movement', 'night_movement', 'towing', 'theft_risk'];
const SCORECARD_DRIVING_ALERT_TYPES = ['speeding', 'harsh_brake', 'harsh_accel', 'harsh_cornering'];
...
router.post('/backups/run', superadminOnly, async (req, res) => {
  ...
  try {
    ...
    await logAudit(req.user.id, 'backup.manual_run', 'backups', null, { bytes: manifest?.postgres?.bytes || null }, req.ip);
