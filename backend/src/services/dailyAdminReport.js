...
const PORTAL = process.env.PORTAL_URL || brand.portalUrl;
...
<tr><td style="background:${C.navy};padding:18px 28px"><span style="font-size:20px;font-weight:700;color:#fff">${brand.name}</span><span style="font-size:13px;color:#93c5fd;margin-left:8px">Admin</span></td></tr>
<tr><td style="padding:24px 28px;color:${C.ink};font-size:14px;line-height:1.5">${body}</td></tr>
<tr><td style="background:#f4f6f9;padding:16px 28px;border-top:1px solid ${C.line};font-size:12px;color:${C.muted}">Sent every morning at 08:00 to ${brand.name} admins. Figures match the agreement pages on <a href="${PORTAL}" style="color:${C.navy}">${brand.domain}</a>.</td></tr>
...
function reportSubject(r) {
  ...
  return `${brand.name} daily report, ${day}: ${bits.join(', ')}`;
}
