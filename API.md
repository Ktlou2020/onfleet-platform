...
3. **Alarms** — receive by webhook. Verify the signature, dedupe on `event_id`, act on `event_type` + `severity`, and call `contact.phone` — OnFleet's office line (010 141 1165) for alarms raised Monday to Friday 08:00–17:00 Johannesburg time, and the after-hours line (081 539 5612) at all other times, including South African public holidays. `driver.phone` carries the same number; `contact` is present even when no rider is assigned.
