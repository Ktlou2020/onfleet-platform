...
router.get('/dashboard', (req, res) => {
  const stats = {
    ...
    revenue_30d: db.prepare(`SELECT COALESCE(SUM(amount),0) s FROM payments WHERE status = 'success' AND paid_at >= datetime('now','-30 days')`).get().s,
  ...
  const weekly = db.prepare(`SELECT strftime('%Y-%W', paid_at) week, COALESCE(SUM(amount),0) total
...
router.get('/users', (req, res) => {
  ...
  const sql = `SELECT id, email, full_name, phone, role, status, created_at
    FROM users
...
router.post('/users', superadminOnly, (req, res) => {
  ...
  const exists = db.prepare('SELECT id FROM users WHERE email = ? AND deleted_at IS NULL').get(email.toLowerCase());
  if (exists) return res.status(409).json({ error: 'Email already exists' });
  const hash = bcrypt.hashSync(password, 10);
  const info = db.prepare(`INSERT INTO users (email, password_hash, full_name, phone, role, status)
    VALUES (?,?,?,?,?, 'active')`).run(email.toLowerCase(), hash, full_name, phone || null, role);
