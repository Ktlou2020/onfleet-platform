...
import { BrainCircuit, TrendingUp, AlertCircle, Bike, Wallet, Download } from 'lucide-react';
...
export default function AdminStrategyReport() {
  ...
  const downloadPdf = async () => {
    const target = document.getElementById('strategy-report-root');
    if (!target) return;
    try {
      setExporting(true);
      const canvas = await html2canvas(target, { scale: 2, useCORS: true, backgroundColor: '#0d1117' });
      ...
      pdf.save(`onfleet-ai-strategy-report-${new Date().toISOString().slice(0, 10)}.pdf`);
    } finally {
      setExporting(false);
    }
  };
    ...
    <>
      <div className="flex-between mb-3" style={{ gap: 16, alignItems: 'flex-start' }}>
        ...
        <div className="row" style={{ flexWrap: 'wrap' }}>
          ...
          <button className="btn btn-secondary" onClick={downloadPdf} disabled={exporting}><Download size={16} /> {exporting ? 'Preparing PDF…' : 'Download PDF'}</button>
        </div>
      </div>

      <div id="strategy-report-root">
